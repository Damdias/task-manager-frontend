# task-data-access Specification

## Purpose

TBD - defines the frontend data-access layer for tasks: the `Task` type definitions, the configured Axios client with shared error parsing, and the TanStack Query hooks (`useTasks`, `useCreateTask`) that are the only layer permitted to call the task API.
